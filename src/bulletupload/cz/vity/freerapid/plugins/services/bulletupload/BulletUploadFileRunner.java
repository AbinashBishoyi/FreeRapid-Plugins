 ...
 * @author tong2shot
 */
class BulletUploadFileRunner extends AbstractRunner {
    private final static Logger logger = Logger.getLogger(BulletUploadFileRunner.class.getName());

    @Override
    public void runCheck() throws Exception { //this method validates file
        super.runCheck();
        addCookie(new Cookie(".bulletupload.com", "lang", "english", "/", null, false));
    ...
    private void checkNameAndSize(String content) throws ErrorDuringDownloadingException {
        PlugUtils.checkName(httpFile, content, "<span class=\"title\">DOWNLOAD: ", " </span>");
        PlugUtils.checkFileSize(httpFile, content, "File Size: </span> ", "&nbsp;");
        httpFile.setFileState(FileState.CHECKED_AND_EXISTING);
    }

    private boolean login() throws Exception {
        synchronized (BulletUploadFileRunner.class) {
            BulletUploadServiceImpl service = (BulletUploadServiceImpl) getPluginService();
            ...
            final HttpMethod httpMethod = getMethodBuilder()
                    .setAction("http://bulletupload.com/login.html")
                    ...
                    .setParameter("submit", "")
                    .toPostMethod();
            addCookie(new Cookie(".bulletupload.com", "login", pa.getUsername(), "/", null, false));
            addCookie(new Cookie(".bulletupload.com", "xfss", "", "/", null, false));
            if (!makeRedirectedRequest(httpMethod))
                throw new ServiceConnectionProblemException("Error posting login info");
            if (getContentAsString().contains("Incorrect Login or Password"))
                throw new NotRecoverableDownloadException("Invalid BulletUpload registered account login information!");
        ...
        if (makeRedirectedRequest(method)) { //we make the main request
            ...
            while (getContentAsString().contains("recaptcha/api/challenge")) {
                MethodBuilder methodBuilder = getMethodBuilder()
                        ...
                        .removeParameter("method_premium");
                if (isPassworded()) {
                    final String password = getDialogSupport().askForPassword("BulletUpload");
                    if (password == null) {
