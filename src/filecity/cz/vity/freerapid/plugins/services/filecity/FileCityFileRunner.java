 ...
 * @author birchie
 */
class FileCityFileRunner extends AbstractRunner {
    private final static Logger logger = Logger.getLogger(FileCityFileRunner.class.getName());
    ...
    private void checkNameAndSize(String content) throws ErrorDuringDownloadingException {
        final Matcher match = PlugUtils.matcher("strong>\\s*?(.+?)\\((.+?[Bb])\\).*?\\s*?</strong", content);
        if (!match.find())
    ...
    public void run() throws Exception {
        ...
        if (makeRedirectedRequest(method)) { //we make the main request
            ...
            final HttpMethod aMethod = getMethodBuilder()
                    .setReferer(fileURL).setActionFromAHrefWhereATagContains("download now").toHttpMethod();
            ...
            while (captchaLoop) {
                ...
                final HttpMethod cMethod = doCaptcha(getMethodBuilder()
                        .setReferer(fileURL).setActionFromFormWhereTagContains("recaptcha", true)).toPostMethod();
