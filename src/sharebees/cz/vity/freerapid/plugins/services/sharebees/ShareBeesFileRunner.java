package cz.vity.freerapid.plugins.services.sharebees;
 ...
 * @author birchie
 */
class ShareBeesFileRunner extends AbstractRunner {
    private final static Logger logger = Logger.getLogger(ShareBeesFileRunner.class.getName());
    ...
    private void checkNameAndSize(String content) throws ErrorDuringDownloadingException {
        ...
        final Matcher matcher = PlugUtils.matcher("\\(([\\s\\d\\.,]+?(?:bytes|.B|.b))\\s*\\)", content);
        if (!matcher.find())
    ...
    public void run() throws Exception {
        ...
        if (makeRedirectedRequest(method)) { //we make the main request
            ...
            final HttpMethod httpMethod1 = getMethodBuilder()
                    .setReferer(fileURL)
                    .setActionFromFormWhereTagContains("down_direct", true)
                    .setAction(fileURL)
                    .toPostMethod();
