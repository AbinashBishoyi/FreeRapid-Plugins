 ...
 * to the {@link com.subgraph.orchid.Circuit#openCircuit(java.util.List, com.subgraph.orchid.CircuitBuildHandler)}
 * method.
 * <p/>
 * The normal sequence of callbacks which are fired when a circuit is opened
 * successfully is {@link #connectionCompleted(com.subgraph.orchid.Connection)} for the initial
 * connection to the entry router, followed by one or more
 * {@link #nodeAdded(com.subgraph.orchid.CircuitNode)} as the circuit is extended with new nodes.
 * When all requested nodes in the path have been added successfully to the
 * circuit {@link #circuitBuildCompleted(com.subgraph.orchid.Circuit)} is called and passed the
 * newly constructed circuit.
 *
 * @see com.subgraph.orchid.Circuit#openCircuit()
 */
