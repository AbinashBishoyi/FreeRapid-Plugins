...
class TurboBitFileRunner extends AbstractRunner {
    ...
    private void checkNameAndSize() throws ErrorDuringDownloadingException {
    	Matcher nameMatcher=PlugUtils.matcher("Скачать файл(?:.*?)<b>(.*?)</b>", getContentAsString());
    	if(!nameMatcher.find())
    		unimplemented();
    ...
    public void run() throws Exception {
        ...
        final HttpMethod method1 = getMethodBuilder()
        	.setActionFromAHrefWhereATagContains("Скачать бесплатно")
        ...
        HttpMethod method2 = getMethodBuilder()
        	.setActionFromFormWhereTagContains("Скачать", true)
        ...
        HttpMethod method4 = getMethodBuilder()
        	.setActionFromAHrefWhereATagContains("Скачать файл")
        	.setBaseURL("http://www.turbobit.net")
        	.toHttpMethod();
      ...
      throw new PluginImplementationException();//some unknown problem
		...
		private void checkFileProblems() throws ErrorDuringDownloadingException {
			...
			if(err404Matcher.find()) {
				if(err404Matcher.group(1).contains("Запрошенный документ не найден"))
    ...
    private void checkDownloadProblems() throws ErrorDuringDownloadingException {
  		try {
    	Matcher waitMatcher=PlugUtils.matcher("Попробуйте\\s+повторить.*<span id='timeout'>([^>]*)<", getContentAsString());
    	if(waitMatcher.find()) {
    	...
    	if(getContentAsString().contains("Ссылка просрочена")) // it's unlikely we get this...
