...
class FrequencyTables implements SBRConstants {
    ...
    private void calculateMFT(SBRHeader header, int sampleRate) throws AACException {
        ...
        if ((k2 - k0) > max)
            throw new AACException("SBR: too many subbands: " + (k2 - k0) + ", maximum number for samplerate " + sampleRate + ": " + max);
        ...
        //check requirement (4.6.18.3.6):
        if (header.getXOverBand(false) >= nMaster)
            throw new AACException("SBR: illegal length of master frequency table: " + nMaster + ", xOverBand: " + header.getXOverBand(false));
    }
